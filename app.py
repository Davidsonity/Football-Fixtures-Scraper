...
if st.button('Fixtures'):
    ...
    url = team_df[(team_df['Team'] == team_) & (team_df['League'] == league_)]['url'].values[0]
    response = requests.get(url, headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Windows; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.5060.114 Safari/537.36'})
    ...
    try:
        ...
        st.warnings('incorect teamname, country or both')
    # # Indexing from 1
